...
function Info(props) {
  ...
  React.useEffect(() => {
    ...
    var data = [];
    for(var i = 0; i < props.tourlist.length; i++){
      if(props.tourlist[i].tourLocation.includes(props.ps[0])
      || props.tourlist[i].tourName.includes(props.ps[0])){


      // if(props.spaces[i].location === props.ps[0]){
        data.push(props.tourlist[i]);
        // console.log("들어감");
  ...
  return (
    <List>
      {items.map(x => (
        <li key={x.tourCode}> {/*x 만 하면 두개 키 오류 나옴 */}
          <Button type="button" onClick={() => toggle(x)}>{`${x.tourName}`}</Button>
